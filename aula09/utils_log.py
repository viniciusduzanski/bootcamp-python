...
def log_decorator(func): # Vou receber uma função como parâmetro
    @wraps(func)
    def wrapper(*args, **kwargs): # Quero receber os argumentos que o usuário passou para a função
        logger.info(f"Chamando função '{func.__name__}' com args {args} e kwargs {kwargs}")
        try:
            result = func(*args, **kwargs) # Executando a função que recebi via parâmetro
